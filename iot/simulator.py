...
STATION_INFORMATION_ID = "urn:ngsi-ld:station_information:acoruna:bicicoruna"
...
def load_stations_from_orion() -> List[Tuple[str, int]]:
    """Load station_id and capacity from station_information entity in Orion-LD."""
    encoded_entity_id = quote(STATION_INFORMATION_ID, safe="")
    endpoint = f"{ORION_URL}/ngsi-ld/v1/entities/{encoded_entity_id}"
    request = Request(
        endpoint,
        headers={
    ...
    data = unwrap(entity.get("data", {}))
    stations_raw = unwrap(data.get("stations", [])) if isinstance(data, dict) else []

    stations: List[Tuple[str, int]] = []
    for station in stations_raw:
        if not isinstance(station, dict):
            continue
        station_id = unwrap(station.get("station_id"))
        capacity_raw = unwrap(station.get("capacity"))
...
def build_payload(num_bikes_available: int, ts_iso: str) -> str:
    payload = {
    ...
    try:
        while True:
            ...
            for station_id, capacity in stations:
                ...
                payload = build_payload(next_value, ts)
