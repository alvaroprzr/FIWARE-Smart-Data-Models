...
def generate_station_status_rows(end_date: datetime) -> List[Tuple[Any, ...]]:
    """Generate 90 days of station_status rows (15 min intervals)."""
    rows = []
    start_date = end_date - timedelta(days=90)
    current_time = start_date
    
    station_index = 0
    for _ in range(90 * 24 * 4 * 15):
...
def generate_weather_rows(end_date: datetime) -> List[Tuple[Any, ...]]:
    """Generate 90 days of weatherobserved rows (hourly)."""
    rows = []
    start_date = end_date - timedelta(days=90)
...
def generate_trips(end_date: datetime) -> List[Tuple[Any, ...]]:
    """Generate 5000 trips distributed by demand peaks."""
    rows = []
    start_date = end_date - timedelta(days=90)
    
    for trip_num in range(5000):
        trip_id = f"TRIP-{trip_num:05d}"
        
        random_time = start_date + timedelta(
            seconds=RNG.randint(0, int(90 * 24 * 3600))
        )
