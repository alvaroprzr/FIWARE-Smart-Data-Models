...
HTTP_CODE=$(curl -sS -o /dev/stderr -w "%{http_code}" -X POST "${IOTAGENT_URL}/iot/services" \
...
echo "Provisioning IoT Agent devices..."
...
EOF
  ...
  DEVICE_CODE=$(curl -sS -o /dev/stderr -w "%{http_code}" -X POST "${IOTAGENT_URL}/iot/devices" \
    -H "Content-Type: application/json" \
    "${FIWARE_HEADERS[@]}" \
    -d "${DEVICE_PAYLOAD}") || true
  if [[ "${DEVICE_CODE}" == "201" || "${DEVICE_CODE}" == "409" ]]; then
    echo "Device ${DEVICE_ID} provisionado o ya existía: ${DEVICE_CODE}"
  else
    echo "Aviso: provisioning de ${DEVICE_ID} devolvió HTTP ${DEVICE_CODE}"
  fi
done
...
create_subscription() {
  ...
  CODE=$(curl -sS -o /dev/stderr -w "%{http_code}" -X POST "${ORION_URL}/ngsi-ld/v1/subscriptions" \
